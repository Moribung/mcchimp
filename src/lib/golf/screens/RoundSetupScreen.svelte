...
<script>
  ...
  const { onstartround, onback } = $props();

  let holeCount     = $state(9);
  ...
  function start() {
    if (!canStart) return;
    onstartround?.({ holeCount, modId: selectedModId });
  }
</script>
...
<div class="setup-wrap">
  <div class="setup-header">
    <h2>New Round</h2>
    <p>Choose your course length and what you'll be quizzed on.</p>
  </div>
  ...
  <div class="actions">
    ...
    <button class="btn btn-primary btn-lg" disabled={!canStart} onclick={start}>Tee Off →</button>
  </div>
</div>
...
<style>
  ...
  .setup-header h2 { font-family: var(--font-display); font-size: 42px; letter-spacing: .03em; color: var(--gold); }
  ...
  .length-btn:hover:not(:disabled) { border-color: var(--gold-dim); }
  .length-btn.active { border-color: var(--gold); background: rgba(212,168,71,.08); }
  .length-btn:disabled { opacity: .4; cursor: not-allowed; }
  .length-num { font-family: var(--font-display); font-size: 34px; color: var(--gold); line-height: 1; }
