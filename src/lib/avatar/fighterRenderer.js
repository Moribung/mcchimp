...
// ── PNG-layer rendering (toggleable; procedural path above is kept for revert) ──
//
// Set RENDER_MODE = 'procedural' to fully restore the old behaviour: drawBase()
// and every constant above are untouched, and no PNGs are fetched. In 'sprites'
// mode the base is composited from the flat-keyed layer PNGs in
// static/sprites/fighter/ (generated by scripts/sprites/gen_fighter_layers.py),
// then run through the SAME recolor() pass. The procedural drawBase() doubles as
// the fallback until the PNGs finish loading — and since the PNGs are pixel-exact
// renders of that art, the switch-over is visually seamless.
export const RENDER_MODE = 'sprites'; // 'sprites' | 'procedural'

const SPRITE_BASE  = '/sprites/fighter';
const SPRITE_FILES = [
  'body_loss', 'body_side', 'body_win1', 'body_win2',
  'shorts_regional', 'shorts_kfc', 'shorts_apex', 'shorts_gfl',
  'belt_gfl', 'belt_apex', 'belt_kfc', 'belt_regional',
  'beard_goatee', 'beard_full',
  'hair_buzz', 'hair_short', 'hair_mohawk', 'hair_afro', 'hair_long',
];
const spriteImgs = {};
let spritesReady   = false;
let preloadStarted = false;

// Kick off a one-time async load of every layer PNG. Idempotent; browser-only.
export function preloadFighterSprites() {
  ...
  for (const name of SPRITE_FILES) {
    const im = new Image();
    im.onload = () => {
      if (++loaded === SPRITE_FILES.length) {
        spritesReady = true;
        baseCache.clear(); // drop procedural-fallback bases so the sprite path takes over
      }
    };
...
// Composite the layer PNGs in draw order: body -> shorts -> belt -> beard -> hair.
function drawBaseSprites(g, cfg) {
  const put = (name) => { const im = spriteImgs[name]; if (im && im.complete) g.drawImage(im, 0, 0); };
  put(`body_${cfg.pose}`);
  put(`shorts_${cfg.org}`);
  if (cfg.belt  && cfg.belt  !== 'none') put(`belt_${cfg.belt}`);
  if (cfg.beard && cfg.beard !== 'none') put(`beard_${cfg.beard}`);
  if (cfg.hair  && cfg.hair  !== 'bald') put(`hair_${cfg.hair}`);
}
...
function getBase(cfg) {
  ...
  if (!c) {
    ...
    // Sprite path once PNGs are loaded; procedural drawBase() is the seamless
    // fallback until then (and the full behaviour when RENDER_MODE === 'procedural').
    if (RENDER_MODE === 'sprites' && spritesReady) drawBaseSprites(g, cfg);
    else drawBase(g, cfg);
    baseCache.set(key, c);
  }
  return c;
...
export function renderFighter(cfg, palette) {
  if (RENDER_MODE === 'sprites') preloadFighterSprites();
